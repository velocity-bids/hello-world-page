...
const AdminDashboard = () => {
  ...
  const { data: reports, isLoading: reportsLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await getReports(
        reportFilter === "all" ? undefined : reportFilter
      );
      if (error) throw error;
      return data;
    },
  ...
  const updateVehicleMutation = useMutation({
    mutationFn: async ({
      ...
      const { error } = await updateVehicleApprovalStatus(
        vehicleId,
        status,
        notes
      );
    ...
    <div className="min-h-screen bg-background">
                          ...
                          paginatedReports.map((report) => (
                            <TableRow key={report.id}>
                              <TableCell>
                                <button
                                  onClick={() =>
                                    navigate(`/vehicle/${report.vehicle_id}`)
                                  }
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {dialogAction === "approve" ? "Approve" : "Decline"} Vehicle
              Listing
            </DialogTitle>
            <DialogDescription>
              {selectedVehicle && (
